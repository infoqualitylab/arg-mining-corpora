    ...
    <Drawer anchor="right" open={open} onClose={onClose}>
      <Box sx={{ width: 1200, height: "100%", display: "flex" }}>
        ...
        >
          ...
          <Stack spacing={0.5}>
            <Typography variant="body2">
              Documents: {row.dataset_document_count}
            </Typography>
            <Typography variant="body2">
              Type: {row.dataset_document_type}
            </Typography>
            <Typography variant="body2">
              Language: {row.dataset_language}
            </Typography>
        ...
        <Box sx={{ width: "60%", p: 3, overflowY: "auto" }}>
          ...
          <Stack spacing={3}>
            {row.annotation_entries.map(
              (entry: AnnotationEntry, idx: number) => {
                const pieData = [
                  { name: "Annotated", value: entry.subset },
                  {
                    name: "Remaining",
                    value: Math.max(
                      row.dataset_document_count - entry.subset,
                      0
                    ),
                  },
                ];
                ...
                return (
                  <Card key={idx} variant="outlined" sx={{ p: 2 }}>
                    ...
                    <Stack direction="row" spacing={2} flexWrap="wrap" mb={2}>
                      ...
                      {entry.paper_link && (
                        <Link href={entry.paper_link}>Paper</Link>
                      )}
                      {entry.open_alex_id && (
                        <Link href={entry.open_alex_id}>OpenAlex</Link>
                      )}
                    </Stack>
